...
### Most Happy songs are in C or C sharp (week 8)

```{r}
ggplotly(ggplot(Happy_Sad, aes(x=key))+
  geom_bar(fill='lightpink', color='pink1')+
  facet_wrap(~playlist)+
  theme_minimal())

 ggplotly(ggplot(happy_playlists, aes(x=key))+
    geom_bar(fill='lightcoral', color='indianred2')+
    facet_wrap(~playlist)+
  theme_minimal())
  
  ggplotly(ggplot(opposite_playlists, aes(x=key))+
